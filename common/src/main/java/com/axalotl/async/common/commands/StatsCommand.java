    ...
    private static void showEntityStats(CommandSourceStack source, int topCount) {
        ...
        server.execute(() -> {
            ...
            server.getAllLevels().forEach(world -> {
                String worldName = world.dimensionTypeRegistration()
                        .unwrapKey()
                        .orElseThrow()
                        .location()
                        .toString();
            ...
            if (topCount > 0) {
                ...
                if (!sortedEntities.isEmpty()) {
                    ...
                    for (Map.Entry<EntityType<?>, Integer> entry : sortedEntities) {
                        ...
                        ResourceLocation nameId = registryAccess.registryOrThrow(Registries.ENTITY_TYPE).getKey(type);
                        String name = Objects.requireNonNull(nameId).toLanguageKey();
